...
class ConexaoDB2 {
    ...
    private static function loadVariaveis() {
        ...
        $env["HOST"] = $_SERVER["HOST"];

        echo 'HOST ENV FIXO: ' . $env["HOST"];
        echo 'variaveis - SERVER: <pre>' . print_r($_SERVER, true) . '</pre>';
        echo 'variaveis - ENV: <pre>' . print_r($_ENV, true) . '</pre>';
    ...
    public static function conecta() {
        if (is_null(self::$conexao)) {
            ...
            if (self::$conexao === false) {
                throw new Exception('Erro ao comunicar com banco de dados!Config: ' . $link . pg_last_error());
            }
        }
        return self::$conexao;
    }
